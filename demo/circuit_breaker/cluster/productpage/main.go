...
var circuitBreakerMap = map[string]*CircuitBreaker{
	detailsUrl: NewCircuitBreaker("details default", time.Second*10, 3, 1),
	reviewsUrl: NewCircuitBreaker("reviews default", time.Second*10, 3, 1),
}

func getData(url string, gremlinHeader string) (string, int) {
	var cb = circuitBreakerMap[url]
	if !cb.CheckIfRequest() {
		return cb.GetDefaultValue(), http.StatusInternalServerError
	}
	...
	if !(err == nil && http.StatusOK <= response.StatusCode && response.StatusCode <= http.StatusIMUsed) {
		fmt.Printf("getData http request error: %v %v", err, response.Status)
		cb.UpdateRequestStatus(false)
		return "unavailable", http.StatusInternalServerError
	}
	defer response.Body.Close()
	cb.UpdateRequestStatus(true)
